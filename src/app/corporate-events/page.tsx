...
import { corporatePage as cp, site } from "../../data";

const iconMap: Record<string, React.ComponentType<{ className?: string }>> = {
  Building2, Wifi, Target, Briefcase, Shield, Users, Star,
};

const corporateFeatures = cp.features.map((f) => ({ ...f, icon: iconMap[f.iconName] || Building2 }));
...
export default function Corporate() {
  ...
  return (
    <>
      <Breadcrumbs
        crumbs={[{ name: "Home", href: "/" }, { name: cp.breadcrumb }]}
      />
            ...
            <div className="relative animate-float">
              ...
              <div className="absolute -top-4 -left-4 bg-white rounded-2xl shadow-2xl p-4 transform rotate-3 hover:rotate-0 transition-transform duration-300 border border-gray-100 animate-pulse-slow z-20">
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    <p className="text-lg font-bold text-gray-900">{cp.floatingCards[0].value}</p>
                    <p className="text-sm text-gray-600">{cp.floatingCards[0].label}</p>
                  </div>
                </div>
              </div>
              ...
              >
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    <p className="text-lg font-bold text-gray-900">{cp.floatingCards[1].value}</p>
                    <p className="text-sm text-gray-600">{cp.floatingCards[1].label}</p>
                  </div>
                </div>
              </div>
            </div>
        ...
        <div className="max-w-7xl mx-auto px-6 lg:px-8">
          <div className="text-center mb-16 animate-slide-up">
            <span className="inline-flex items-center bg-white/80 text-blue-600 px-4 py-2 rounded-full text-sm font-semibold shadow-sm mb-6">
              ...
              {cp.featuresSectionBadge}
            </span>
            <h2 className="text-4xl lg:text-5xl font-bold text-gray-900 mb-4">
              {cp.featuresSectionHeading.line1}{" "}
              <span className="text-blue-600">{cp.featuresSectionHeading.highlight}</span>
            </h2>
            <p className="text-xl text-gray-600 max-w-3xl mx-auto">
              {cp.featuresSectionDescription}
            </p>
          </div>
            ...
            <div className="space-y-8 animate-slide-up">
              <div className="space-y-6">
                ...
                <p className="text-lg text-gray-700 leading-relaxed">
                  {cp.detailDescription}
                </p>
              </div>

              {/* Features Grid */}
              <div className="grid grid-cols-1 gap-4">
                {cp.detailFeatures.map((feature, index) => (
                  <div
                    key={index}
              ...
              <div className="pt-6">
                <h3 className="text-2xl font-bold text-gray-900 mb-4">
                  {cp.destinationsHeading}
                </h3>
                <div className="grid grid-cols-2 gap-3">
                  {cp.destinations.map((destination, index) => (
                    <div
                      key={index}
            ...
            >
              ...
              <div className="absolute -bottom-6 -right-6 bg-white rounded-2xl shadow-2xl p-6 border border-gray-100 animate-pulse-slow">
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    <p className="text-lg font-bold text-gray-900">{cp.floatingInfoCard.value}</p>
                    <p className="text-sm text-gray-600">{cp.floatingInfoCard.label}</p>
                  </div>
                </div>
              </div>
            </div>
        ...
        <div className="max-w-4xl mx-auto px-6 lg:px-8 text-center">
          <div className="space-y-6 animate-slide-up">
            ...
            <h2 className="text-4xl lg:text-5xl font-bold text-gray-900">
              {cp.ctaHeading.line1}{" "}
              <span className="text-blue-600">{cp.ctaHeading.highlight}</span>
            </h2>
